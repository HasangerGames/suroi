...
## About
Suroi 是一个开源的2D吃鸡大逃杀游戏，灵感来自 [surviv.io](https://survivio.fandom.com/wiki/Surviv.io_Wiki).目前，这个开源项目还在保持更新。

## 游玩游戏！
[suroi.io](https://suroi.io)

## 捐赠！
任何金额都有帮助！所有捐款都用于维持服务器的运行。
...
## 设置以及安装
首先您需要安装： [Node.js](https://nodejs.org) 和 [pnpm](https://pnpm.io).

接下来, [点我之后下载部署用的东西](https://github.com/HasangerGames/suroi/archive/refs/heads/master.zip),或使用下面的代码去git获得
```sh
git clone https://github.com/HasangerGames/suroi.git
```

要安装依赖项，请在项目根目录中打开一个终端，然后跑下面的命令：
```sh
pnpm install
```

这将在所有三个文件夹（“client”、“common”和“server”）中创建一个“node_modules”目录，并将包链接到那里。

## 运行
要在本地运行游戏，请在项目根目录中打开一个终端并运行以下命令：

```sh
pnpm dev
```
想要进行游玩, 只需要在您的浏览器打开http://127.0.0.1:3000。
