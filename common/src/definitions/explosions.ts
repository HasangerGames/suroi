...
import { inheritFrom, ObjectDefinitions, type BaseBulletDefinition, type KillfeedItemMixin, type ObjectDefinition, type RawDefinition, type ReferenceTo } from "../utils/objectDefinitions";
...
export const Explosions = ObjectDefinitions.withDefault<ExplosionDefinition>()(
    ...
    ([, , , missing]) => {
        ...
        ] satisfies ReadonlyArray<RawDefinition<WithPartial<Missing, "idString" | "killfeedFrame">>>).map(def => {
            def.idString ??= `${def.name.toLowerCase().replace(/ /g, "_")}_explosion`;
            def.killfeedFrame ??= def.idString;

            return def as Missing;
        });
    }
);
